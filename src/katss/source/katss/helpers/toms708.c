 ...
 * constructs such as GOTOs and macros with structured programming constructs.
 *
 */
...
static double r_log1_exp(double x) {
...
static double r_d__0(bool log_p) { return log_p ? ML_NEGINF : 0.0; }

static double r_d__1(bool log_p) { return log_p ? 0.0 : 1.0; }

static double r_d_exp(bool log_p, double x) { return log_p ? x : exp(x); }
...
static void l_end(double *w, double *w1, bool do_swap) {
  if (do_swap) {
    double t = *w;
    *w = *w1;
    *w1 = t;
...
static void l_end_from_w(double *w, double *w1, bool do_swap, bool log_p) {
  if (log_p) {
    *w1 = log1p(-*w);
    *w = log(*w);
  } else {
...
static void l_end_from_w1(double *w, double *w1, bool do_swap, bool log_p) {
  if (log_p) {
    *w = log1p(-*w1);
    *w1 = log(*w1);
  } else {
...
static void l_end_from_w1_log(double *w, double *w1, bool do_swap, bool log_p) {
...
static void l_w_bpser(double a0, double b0, double x0, double *w, double *w1,
...
static void l_w1_bpser(double a0, double b0, double y0, double *w, double *w1,
                double eps, bool do_swap, bool log_p) {
...
static void l_bfrac(double a0, double b0, double x0, double y0, double lambda,
...
static void l140(double a0, double b0, double x0, double y0, double eps, double *w,
  ...
  *w = bup(b0, a0, y0, x0, n, eps, false);
  ...
  if (*w < DBL_MIN && log_p) { /* do not believe it; try bpser() : */
    ...
    l_w_bpser(a0, b0, x0, w, w1, eps, do_swap, log_p);
	return;
  }
...
static void l131(double a, double b, double x, int n, double a0, double b0, double x0,
          double y0, double *w, double *w1, double eps, int *ierr, int ierr1,
          bool did_bup, bool do_swap, bool log_p) {
...
#endif
    l_end_from_w1_log(w, w1, do_swap, log_p);
	return;
  }
  // else
  if (ierr1) {
...
static void bratio_final_else(double a, double b, double x, double y, double eps,
                       double *w, double *w1, int *ierr, int ierr1,
                       bool log_p) {
  ...
  if (b0 < 40.) {
    ...
    if (b0 * x0 <= 0.7 ||
      ...
      l140(a0, b0, x0, y0, eps, w, w1, do_swap, ierr, ierr1, log_p);
	  return;
    }
  } else if (a0 > b0) { /* ----  a0 > b0 >= 40  ---- */
    R_ifDEBUG_printf("  a0 > b0 >= 40;");
    if (b0 <= 100. || lambda > b0 * 0.03) {
      l_bfrac(a0, b0, x0, y0, lambda, eps, w, w1, do_swap, log_p);
	  return;
    }
  } else if (a0 <= 100.) {
  ...
  if (min(a, b) <= 1.) { /*------------------------ a <= 1  or  b <= 1 ---- */
    ...
    if (max(a0, b0) > 1.) { /* L20:  min(a,b) <= 1 < max(a,b)  */
      ...
      if (x0 >= 0.29) {/* was 0.3, PR#13786 */
        l_w1_bpser(a0, b0, y0, w, w1, eps, do_swap, log_p);
		return;
	  }
      ...
      if (b0 > 15.) {
        *w1 = 0.;
        l131(a, b, x, n, a0, b0, x0, y0, w, w1, eps, ierr, ierr1,
    ...
    *w1 = bup(b0, a0, y0, x0, n, eps, false);
    ...
    b0 += n;
    l131(a, b, x, n, a0, b0, x0, y0, w, w1, eps, ierr, ierr1, did_bup,
                do_swap, log_p);
	return;
...
static double fpser(double a, double b, double x, double eps, int log_p) {
  /* ----------------------------------------------------------------------- *
...
static double l40(double a, double b, double w) {
